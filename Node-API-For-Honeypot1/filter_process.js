    ...
    while(i<filter1.length){
        ...
        if (filter1[i].ip === beforeIp && splitDate[0] === beforeDate[0] && splitDate[1] === beforeDate[1] && (parseInt(splitDate[2]) <= parseInt(beforeDate[2])+5) ){ //+5 5 dakika içinde !!
            ...
            if(alertCounter > 100){
                ...
                alertIp.push({pid:i+1,ip:filter1[i].ip,date:filter1[i].date,method:filter1[i].method,path:filter1[i].path,status:filter1[i].status,alertLevel:"High",alertDescription:"The attack this attack may be a dictionary attack"}) //tehlikeli ip adresleri bulundu !!!
                alertCounter = 0
            } 
        }
        
        if (filter1[i].path === "/files/"){
                //console.log(" Tehlikeli Ip Adresi Bulundu !!!!")
                ...
                alertIp.push({pid:i+1,ip:filter1[i].ip,date:filter1[i].date,method:filter1[i].method,path:filter1[i].path,status:filter1[i].status,alertLevel:"Very High",alertDescription:"The Honeypot path is found!! The attacker is here"})//tehlikeli ip adresleri bulundu !!!
        }
...
fs.watchFile("./honeypot1Log/apache2/access.log", async (curr, prev)=> {        // file'da bir değişiklik olduğunda tetiklenir
